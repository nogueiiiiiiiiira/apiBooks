...
async function postReturn(req, res) {
  ...
  try {
    ...
    const prevDev = new Date(loan.dataDev).toLocaleDateString('pt-BR', { year: 'numeric', month: '2-digit', day: '2-digit' });
    ...
    if(multaAtribuida === 'Sim'){
        ...
        await addFine(cpf, idLivro, diasAtra.toString(), total.toString(), statusPag, criadoEm);          
